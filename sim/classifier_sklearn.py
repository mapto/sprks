...
from models.pw_policy import pw_policy_model as policy_model
...
class classifier_sklearn:
    def __init__(self):
        ...
        for risk in self.risks_set:
            ...
            self.incidents_models[risk] = self.train_classifier(risk, limit, general)
    ...
    def train_classifier(self, risk, limit, general):
        filename = glob.glob('static/data/pw-train-generated-risk-'+risk+'.csv')
        data = numpy.genfromtxt(filename[0], delimiter=',')
        data = numpy.concatenate((data, general)) # add positive cases that need to contrast negative ones
        train_data = data[:, 2:(limit + 2)] # first several columns represent the data dimension
        train_result = data[:, (limit + 2)] # result columns are ones after data dimensions
